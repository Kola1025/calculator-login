...
# Creating result text field
                self.resultField = Text(master, bg="#9FFB88", fg="#30D5C8", height=1, width=20)
...
# Creating and aligning calculation buttons
                ...
                bExit = Button(master, text="Exit", command=self.exit)
                bCalculate.grid(row=5, column=1, columnspan=2)
                bClear.grid(row=5, column=2, columnspan=2)
                bExit.grid(row=5, column=0, columnspan=2)
        def notice(self, num):
